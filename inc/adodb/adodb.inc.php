	...
	function &_Execute($sql,$inputarr=false)
	{
		...
		*************************/
		...
		if ($this->_queryID === false) { // error handling if query fails
			...
			//si hay un error lo guardo en el LOG
			if (isset($_SERVER['QUERY_STRING'])) {$variables = '?'.$_SERVER['QUERY_STRING'];}
			else {$variables = '';}
			
			if (isset($_SESSION['nombre']))
				$usuario = $_SESSION['nombre'].' (id: '.$_SESSION['id_usuario'].')';
			else
				$usuario = '-';
					
			$fp=fopen("c:\log_sistema\errores_bd.txt",'a');
			//$fp=fopen("C:\Temp\log\_errores_ocmi.txt",'a');
			fwrite($fp,"-----------------------------------------------------------------------\n");
			fwrite($fp,"Fecha: ".date('Y-m-d H:i:s')."\n");
			fwrite($fp,"Pagina: ".'http://'.$_SERVER['HTTP_HOST'].$_SERVER['SCRIPT_NAME'].$variables."\n");
			fwrite($fp,"Usuario: $usuario \n");
			fwrite($fp,"IP: ".$_SERVER['REMOTE_ADDR']." \n");
			fwrite($fp,"Navegador: ".$_SERVER['HTTP_USER_AGENT']." \n");
			fwrite($fp,"Error: \n");			
			fwrite($fp,"Nro: ".$this->ErrorNo()."\n");
			fwrite($fp,$this->ErrorMsg()."\n");
			fwrite($fp,"SQL: \n");
			fwrite($fp,$sql."\n");
			fclose($fp);
