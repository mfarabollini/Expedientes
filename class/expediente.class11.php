...
class Expediente
{
	...
	public function CargaTituloExp($comisionBD)
	{
		...
		$arrC12 = split(" ",$this->lista12);
			...
			/*print $arrCom[0];
			print $arrC1[0];*/
			if (strcmp(strtoupper($arrCom[0]),strtoupper($arrC1[0])) == 0)
				$titulo = $this->lista1;
			elseif (strcmp(strtoupper($arrCom[0]),strtoupper($arrC2[0])) == 0)	
				$titulo = $this->lista2;
			elseif (strcmp(strtoupper($arrCom[0]),strtoupper($arrC3[0])) == 0)	
				$titulo = $this->lista3;
			elseif (strcmp(strtoupper($arrCom[0]),strtoupper($arrC4[0])) == 0)	
				$titulo = $this->lista4;
			elseif (strcmp(strtoupper($arrCom[0]),strtoupper($arrC5[0])) == 0)	
				$titulo = $this->lista5;
			elseif (strcmp(strtoupper($arrCom[0]),strtoupper($arrC6[0])) == 0)	
				$titulo = $this->lista6;
			elseif (strcmp(strtoupper($arrCom[0]),strtoupper($arrC7[0])) == 0)	
				$titulo = $this->lista7;
			elseif (strcmp(strtoupper($arrCom[0]),strtoupper($arrC8[0])) == 0)	
				$titulo = $this->lista8;
			elseif (strcmp(strtoupper($arrCom[0]),strtoupper($arrC9[0])) == 0)	
				$titulo = $this->lista9;
			elseif (strcmp(strtoupper($arrCom[0]),strtoupper($arrC10[0])) == 0)	
				$titulo = $this->lista10;
			elseif (strcmp(strtoupper($arrCom[0]),strtoupper($arrC11[0])) == 0)	
				$titulo = $this->lista11;
			elseif (strcmp(strtoupper($arrCom[0]),strtoupper($arrC12[0])) == 0)	
				$titulo = $this->lista12;
			else
				$titulo = "Sin Titulo";																							
	...
	public function SqlCheckExpOrd($exp)
	{
		...
		$sql = "select A.tipo_proy, B.causante, A.numero, A.letra, A.anio, C.destino, A.caratula ".
				...
				"and A.numero in (".$exp.")".
				" and  (".
						"SUBSTRING(C.destino,1,8)='GOBIERNO' OR ".
...
//			$fec_alta_valor = '';	
//			if($registro_nuevo == 1) {
//				$fec_alta_campo =  ', fec_alta';
//				$fec_alta_valor = ', ' . $this->fec_alta;
//			}
//		
//			$sqlTemp = "INSERT INTO expedientes_temporal ".
//				   "(numero, registro_nuevo, mod_time, tipo, letra, anio, nro_municipal, tipo_proy, num_mensaje, id_causante, caratula, fec_presentacion, ".
//				   "fec_sesion, com_destino, id_ubicacion_actual, ".
//				   "fec_aprobacion, id_aprobacion, tipo_aprobacion, id_usuario_alta, fec_mod, agregados, id_categoria, impreso, id_grupo " .
//				   $fec_alta_campo .
//				   ") ".
//				   "VALUES ".
//				   "($this->numero, $registro_nuevo, $mod_time, '$this->tipo', '$this->letra', '$this->anio', '$this->nro_municipal', '$this->tipo_proy', '$this->num_mensaje', $this->id_causante, '$this->caratula', $this->fec_presentacion, ".
//				   "$this->fec_sesion, $this->com_destino, $this->id_ubicacion_actual, ".
//				   "$this->fec_aprobacion, $this->id_aprobacion, '$this->tipo_aprobacion', '$this->id_usuario_alta', $this->fec_mod, '$this->agregados', $this->id_categoria, '$this->impreso', $this->id_grupo " .
//				   $fec_alta_valor . ")";
//			
